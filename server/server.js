...
server.post("/signup", async (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(403).json({
      error:
        "Password should be 6 to 20 characters long with a numeric, 1 lowercase and 1 uppdercase character",
    });
  }

  // Check if email already exists
  const isEmailExists = await User.exists({ "personal_info.email": email });
...
server.post("/google-auth", async (req, res) => {
  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      let user = await User.findOne({ "person_info.email": email })
        .select(
          "person_info.fullname person_info.username person_info.profile_img google_auth"
        )
        .then((u) => {
          return u || null;
        })
